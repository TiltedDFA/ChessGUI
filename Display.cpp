...
{
	auto& texture = m_s_instance->m_piece_spr_sheet;
	...
	for(auto i = board.cbegin(); i < board.cend(); ++i)
	{
		if (*i != nullptr)
		{
			...
			sf::Sprite temp_piece(texture,{piece_position.x,piece_position.y,100,100});
			const int index = ( &(*i) - &board[0]) + 1;
			const int column = index % 8;
			const int row = (index - column) / 8;
			temp_piece.setPosition({ (SCREEN_WIDTH / 8) * row,(SCREEN_HIGHT / 8) * column });
